...
void ClientCheatStatusChanged(int clientNum);
void ConnectionlessPacketReceived(byte IP0, byte IP1, byte IP2, byte IP3);
...
int vmMain( int command, int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8, int arg9, int arg10, int arg11  ) {
	...
	case GAME_ROFF_NOTETRACK_CALLBACK:
		...
		return 0;
	// JK2MF: receiving the JK2MF_CLIENT_CHEATSTATUS_CHANGED event
	case JK2MF_CLIENT_CHEATSTATUS_CHANGED:
		ClientCheatStatusChanged(arg0);
		return 0;
	// JK2MF: receiving the JK2MF_CONNECTIONLESS_PACKET event
	case JK2MF_CONNECTIONLESS_PACKET:
		ConnectionlessPacketReceived(arg0, arg1, arg2, arg3);
		return 0;
...
void ConnectionlessPacketReceived(byte IP0, byte IP1, byte IP2, byte IP3)
{
	...
	if ( !strcmp(cmd, "multiply") )
	{
		int n1, n2;
	
		trap_Argv(1, num1, sizeof(num1));
		trap_Argv(2, num2, sizeof(num2));
		n1 = atoi(num1);
		n2 = atoi(num2);
		
		// Send result
		trap_JK2MF_SendConnectionlessPacket(va("result %i", n1 * n2));
	}
}
...
void ClientCheatStatusChanged(int clientNum)
{
	...
	// Trying to get players status from the JK2MF module
	// We need to abort if it returns qfalse
	if ( !trap_JK2MF_GetPlayerStats(clientNum, &pl) )
		return;
    
	// everything is defined in "jk2mf_api.h"
	// look into it for more details
	if ( pl.cheatStatus == CHEAT_FREE )
		trap_SendServerCommand(clientNum, "print \"Yay! You are a good client!\n\"");

	trap_SendServerCommand(clientNum, va("print \"Your cheat status has changed to %i\n"
...
void G_InitGame( int levelTime, int randomSeed, int restart ) {
	...
	// If this server runs JK2MF disable the blocking of player movement
	// if a player is cheating because this is handled another way in this mod. (banning lol)
	if ( JK2MFApiAvailable() )
	{
		trap_JK2MF_SwitchCmdBlocking(qfalse);
	}
	else
	{
		G_Printf("MyMOD: JK2MF is not running.");
		G_Printf("MyMOD: Some features can not be used.");
	}
