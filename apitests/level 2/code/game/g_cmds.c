...
void ClientCommand( int clientNum ) {
	...
	*/
	...
	// This is a client command in the "g_cmds.c" file ( ClientCommand(int) )
	else if ( Q_stricmp(cmd, "uptime") == 0 )
	{
		char buff[100];
  
		// If you want your mod to be backwards compatible without JK2MF you need to check if it is available before every
		// call to a JK2MF specific function.
		if ( !JK2MFApiAvailable() )
		{
			trap_SendServerCommand(clientNum, va("print \"The server needs at least JK2MF %s to make this command available.\n\"", JK2MFMINVERSION));
			return;
		}
  
		trap_JK2MF_GetUptimeString(buff, sizeof(buff));  
		trap_SendServerCommand(clientNum, va("print \"%s ( %i seconds )\n\"", buff, trap_JK2MF_GetUptimeSeconds()));
	}
	
	// This is a client command in the "g_cmds.c" file ( ClientCommand(int) )
	else if ( Q_stricmp(cmd, "myOS") == 0 )
	{
		mfPlayer ply;
		...
		if ( !trap_JK2MF_GetPlayerStats(clientNum, &ply) )
		{
			trap_SendServerCommand(clientNum, "print \"Something went wrong.\n\"");
			return;
		}
		
		if ( ply.clientOS == OS_WINDOWS )
			trap_SendServerCommand(clientNum, "print \"You are using Microsoft Windows.\n\"");
		else if ( ply.clientOS == OS_MAC )
			trap_SendServerCommand(clientNum, "print \"You are using Apple's Mac.\n\"");
		else
			trap_SendServerCommand(clientNum, "print \"I can't tell you what OS you are using.\n\"");
	}
    
    else if ( Q_stricmp(cmd, "banme") == 0 )
    {
        mfPlayer ply;
		...
		if ( !JK2MFApiAvailable() )
		{
			trap_SendServerCommand(clientNum, va("print \"The server needs at least JK2MF %s for this command.\n\"", JK2MFMINVERSION));
			return;
		}
        ...
        // Remember that the list of blocked IP's is cleared on every map load and map_restart
        // so if you want to keep the blocking active you need to have some routine in GAME_INIT.
        if ( trap_JK2MF_BlockIP(ply.IP) )
        {
            trap_DropClient(clientNum, G_GetStripEdString("SVINGAME", "WAS_KICKED"));
        }
        else
        {
            trap_SendServerCommand(clientNum, "print \"For some reason you can't ban yourself in the moment.\n\"");
        }
    }
