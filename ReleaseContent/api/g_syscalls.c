...
{
  return (qboolean)syscall(JK2MF_GET_PLAYERSTATS, clientNum, reqPly);
}

uint trap_JK2MF_GetUptimeSeconds(void)
{
  return (uint)syscall(JK2MF_GET_UPTIMESECS);
}

void trap_JK2MF_GetUptimeString(char *buffer, int bufferSize)
{
  syscall(JK2MF_GET_UPTIMESTR, buffer, bufferSize);
}

void trap_JK2MF_SwitchCmdBlocking(qboolean enableBlocking)
{
  syscall(JK2MF_SWITCH_CMD_BLOCKING, enableBlocking);
}

void trap_JK2MF_SendConnectionlessPacket(const char *message)
{
  syscall(JK2MF_SEND_CONNECTIONLESSPACKET, message);
}

void trap_JK2MF_GetDownloadRequestInfo(mfDownloadInfo *downInf)
{
  syscall(JK2MF_GET_DOWNLOADREQUEST_INFO, downInf);
}

qboolean trap_JK2MF_BlockIP(byte *IP)
{
  return (qboolean)syscall(JK2MF_BLOCK_IP, IP);
}

qboolean trap_JK2MF_UnblockIP(byte *IP)
{
  return (qboolean)syscall(JK2MF_UNBLOCK_IP, IP);
}

void trap_JK2MF_SendConnectionlessPacketIP(byte *IP, int port, const char *message)
{
  syscall(JK2MF_SEND_CONNECTIONLESSPACKET_IP, IP, port, message);
}
