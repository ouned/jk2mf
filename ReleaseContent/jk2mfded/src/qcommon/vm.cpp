...
void VM_LoadSymbols( vm_t *vm ) {
	...
	while ( 1 ) {
		...
		chars = strlen( token );
		sym = (struct vmSymbol_s *)Hunk_Alloc( sizeof( *sym ) + chars, h_high );
		*prev = sym;
		prev = &sym->next;
		sym->next = NULL;
...
vm_t	*VM_Create(const char *module, int(*systemCalls)(int *),
	...
	if (JK2MF_InUse())
		return JK2MF_VM_Create(module, systemCalls, interpret);
	else
		return VM_Create_Ori(module, systemCalls, interpret);
}

vm_t *VM_Create_Ori( const char *module, int (*systemCalls)(int *),
				vmInterpret_t interpret ) {
	vm_t		*vm;
	vmHeader_t	*header;
	int			length;
...
void *VM_ExplicitArgPtr(vm_t *vm, int intValue) {
	if (JK2MF_InUse())
		return JK2MF_VM_ExplicitArgPtr(vm, intValue);
	else
		return VM_ExplicitArgPtr_Ori(vm, intValue);
}

void *VM_ExplicitArgPtr_Ori( vm_t *vm, int intValue ) {
	if ( !intValue ) {
		return NULL;
	}
...
int	QDECL VM_Call(vm_t *vm, int callnum, ...) {
	...
	if (JK2MF_InUse())
		return JK2MF_VM_Call(vm, callnum, args[0], args[1], args[2], args[3],
										  args[4], args[5], args[6], args[7],
										  args[8], args[9], args[10], args[11],
										  args[12], args[13], args[14], args[15]);
	else
		return VM_Call_Ori(vm, callnum, args[0], args[1], args[2], args[3],
										args[4], args[5], args[6], args[7],
										args[8], args[9], args[10], args[11],
										args[12], args[13], args[14], args[15]);
...
int	QDECL VM_Call_Ori( vm_t *vm, int callnum, ... ) {
	vm_t	*oldVM;
	int		r;
	int i;
	int args[16];
...
#ifdef IS64BIT // bk010215 - for DLL_ONLY dedicated servers/builds w/o VM
int	VM_CallCompiled(vm_t *vm, int *args) {
	return(0);
}
