...
searchpath_t **FS_GetSearchPath() { return &fs_searchpaths; }
...
int FS_FOpenFileRead( const char *filename, fileHandle_t *file, qboolean uniqueFILE ) {
	...
	// Com_sprintf (demoExt, sizeof(demoExt), ".dm_%d",PROTOCOL_VERSION ); // fuck that, this is serverside only
...
const char *FS_ReferencedPakChecksums(void) {
	if (JK2MF_InUse())
		return JK2MF_FS_ReferencedPakChecksums();
	else
		return FS_ReferencedPakChecksums_Ori();
}
...
const char *FS_ReferencedPakNames(void) {
	if (JK2MF_InUse())
		return JK2MF_FS_ReferencedPakNames();
	else
		return FS_ReferencedPakNames_Ori();
}
