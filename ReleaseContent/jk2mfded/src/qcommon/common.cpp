...
void QDECL Com_DPrintf(const char *fmt, ...) {
	...
	if (JK2MF_InUse())
		JK2MF_Com_DPrintf("%s", msg);
	else
		Com_DPrintf_Ori("%s", msg);
}

void QDECL Com_DPrintf_Ori( const char *fmt, ...) {
	va_list		argptr;
	char		msg[MAXPRINTMSG];
...
void Com_Init( char *commandLine ) {
	char	*s;
	cvar_t *jk2mfded_force102;

	Com_Printf("JK2MFDED v%s %s %s\n", JK2MFDED_VERSION, CPUSTRING, __DATE__);
	Com_Printf("http://jk2.ouned.de/jk2mf\n");
	...
	//	Swap_Init ();
		...
		jk2mfded_force102 = Cvar_Get("jk2mfded_force102", "0", CVAR_INIT);
		if (FS_Base_FileExists("assets5.pk3") && !jk2mfded_force102->integer) {
			Com_Printf("Setting game version to 1.04...\n");
			JK2MF_SetGameVersion(JK2_104);
		}
		else {
			Com_Printf("Setting game version to 1.02...\n");
			JK2MF_SetGameVersion(JK2_102);
		}

        Com_Printf("Initializing JK2MF...\n");
        if (JK2MF_Init())
            Com_Printf("Loading JK2MF failed\n");
        else
            Com_Printf("JK2MF successfully loaded\n");
	  ...
	  // get dedicated here for proper hunk megs initialization
		...
		if ( JK2MF_GetGameVersion() == JK2_102 )
			s = va("%s %s %s", JK2VERSION_102, CPUSTRING, __DATE__);
		else
			s = va("%s %s %s", JK2VERSION_104, CPUSTRING, __DATE__);
