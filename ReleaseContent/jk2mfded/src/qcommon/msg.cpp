...
#include "../server/jk2mf.h"

char *MSG_ReadStringLine(msg_t *msg) {
	if (JK2MF_InUse())
		return JK2MF_MSG_ReadStringLine(msg);
	else
		return MSG_ReadStringLine_Ori(msg);
}

char *MSG_ReadStringLine_Ori( msg_t *msg ) {
...
netField_t	entityStateFields102[] =
{
	{ NETF(pos.trTime), 32 },
	{ NETF(pos.trBase[0]), 0 },
	{ NETF(pos.trBase[1]), 0 },
...
netField_t	entityStateFields104[] = 
{
{ NETF(pos.trTime), 32 },
{ NETF(pos.trBase[0]), 0 },
{ NETF(pos.trBase[1]), 0 },
...
void MSG_WriteDeltaEntity( msg_t *msg, struct entityState_s *from, struct entityState_s *to, 
	...
	if ( JK2MF_GetGameVersion() == JK2_102 )
		numFields = sizeof(entityStateFields102)/sizeof(entityStateFields102[0]);
	else
		numFields = sizeof(entityStateFields104) / sizeof(entityStateFields104[0]);
	...
	if (JK2MF_GetGameVersion() == JK2_102)
		field = entityStateFields102;
	else
		field = entityStateFields104;
...
//			pcount[i]++;
	...
	if (JK2MF_GetGameVersion() == JK2_102)
		field = &entityStateFields102[lc];
	else
		field = &entityStateFields104[lc];
...
#define	PSF102(x) #x,(int)&((playerState102_t*)0)->x

netField_t	playerStateFields102[] =
{
	{ PSF102(commandTime), 32 },
	{ PSF102(origin[0]), 0 },
	{ PSF102(origin[1]), 0 },
	{ PSF102(bobCycle), 8 },
	{ PSF102(velocity[0]), 0 },
	{ PSF102(velocity[1]), 0 },
	{ PSF102(viewangles[1]), 0 },
	{ PSF102(viewangles[0]), 0 },
	{ PSF102(weaponTime), -16 },
	{ PSF102(weaponChargeTime), 32 }, //? really need 32 bits??
	{ PSF102(weaponChargeSubtractTime), 32 }, //? really need 32 bits??
	{ PSF102(origin[2]), 0 },
	{ PSF102(velocity[2]), 0 },
	{ PSF102(pm_time), -16 },
	{ PSF102(eventSequence), 16 },
	{ PSF102(torsoAnim), 16 },			// Maximum number of animation sequences is 2048.  Top bit is reserved for the togglebit
	{ PSF102(torsoTimer), 16 },
	{ PSF102(legsAnim), 16 },			// Maximum number of animation sequences is 2048.  Top bit is reserved for the togglebit
	{ PSF102(legsTimer), 16 },
	{ PSF102(movementDir), 4 },
	{ PSF102(events[0]), 10 },			// There is a maximum of 256 events (8 bits transmission, 2 high bits for uniqueness)
	{ PSF102(events[1]), 10 },			// There is a maximum of 256 events (8 bits transmission, 2 high bits for uniqueness)
	{ PSF102(pm_flags), 16 },
	{ PSF102(groundEntityNum), GENTITYNUM_BITS },
	{ PSF102(weaponstate), 4 },
	{ PSF102(eFlags), 32 },
	{ PSF102(externalEvent), 10 },
	{ PSF102(gravity), 16 },
	{ PSF102(speed), 16 },
	{ PSF102(basespeed), 16 },
	{ PSF102(delta_angles[1]), 16 },
	{ PSF102(externalEventParm), 8 },
	{ PSF102(viewheight), -8 },
	{ PSF102(damageEvent), 8 },
	{ PSF102(damageYaw), 8 },
	{ PSF102(damagePitch), 8 },
	{ PSF102(damageCount), 8 },
	{ PSF102(damageType), 2 },
	{ PSF102(generic1), 8 },
	{ PSF102(pm_type), 8 },
	{ PSF102(delta_angles[0]), 16 },
	{ PSF102(delta_angles[2]), 16 },
	{ PSF102(eventParms[0]), -16 },
	{ PSF102(eventParms[1]), 8 },
	{ PSF102(clientNum), 8 },
	{ PSF102(weapon), 5 },
	{ PSF102(viewangles[2]), 0 },
	{ PSF102(jumppad_ent), 10 },
	{ PSF102(loopSound), 16 },
	...
	{ PSF102(fd.forcePowersActive), 32 },
	{ PSF102(fd.forceMindtrickTargetIndex), 16 }, //NOTE: Not just an index, used as a (1 << val) bitflag for up to 16 clients
	{ PSF102(fd.forceMindtrickTargetIndex2), 16 }, //NOTE: Not just an index, used as a (1 << val) bitflag for up to 16 clients
	{ PSF102(fd.forceMindtrickTargetIndex3), 16 }, //NOTE: Not just an index, used as a (1 << val) bitflag for up to 16 clients
	{ PSF102(fd.forceMindtrickTargetIndex4), 16 }, //NOTE: Not just an index, used as a (1 << val) bitflag for up to 16 clients
	{ PSF102(fd.forceJumpZStart), 0 },
	{ PSF102(fd.forcePowerSelected), 8 },
	{ PSF102(fd.forcePowersKnown), 32 },
	{ PSF102(fd.forcePower), 8 },
	{ PSF102(fd.forceSide), 2 }, //so we know if we should apply greyed out shaders to dark/light force enlightenment
	{ PSF102(fd.sentryDeployed), 1 },
	{ PSF102(fd.forcePowerLevel[FP_LEVITATION]), 2 }, //unfortunately we need this for fall damage calculation (client needs to know the distance for the fall noise)
	{ PSF102(fd.forcePowerLevel[FP_SEE]), 2 }, //needed for knowing when to display players through walls
	{ PSF102(genericEnemyIndex), 32 }, //NOTE: This isn't just an index all the time, it's often used as a time value, and thus needs 32 bits
	{ PSF102(activeForcePass), 6 },
	{ PSF102(hasDetPackPlanted), 1 },
	{ PSF102(emplacedIndex), GENTITYNUM_BITS },
	{ PSF102(fd.forceRageRecoveryTime), 32 },
	{ PSF102(rocketLockIndex), 8 }, //should never exceed MAX_CLIENTS
	{ PSF102(rocketLockTime), 32 },
	{ PSF102(rocketTargetTime), 32 },
	{ PSF102(holocronBits), 32 },
	{ PSF102(isJediMaster), 1 },
	{ PSF102(fallingToDeath), 32 },
	{ PSF102(electrifyTime), 32 },

	{ PSF102(fd.forcePowerDebounce[FP_LEVITATION]), 32 },

	{ PSF102(saberMove), 32 }, //This value sometimes exceeds the max LS_ value and gets set to a crazy amount, so it needs 32 bits
	{ PSF102(saberActive), 1 },
	{ PSF102(saberInFlight), 1 },
	{ PSF102(saberBlocked), 8 },
	{ PSF102(saberEntityNum), GENTITYNUM_BITS }, //Also used for channel tracker storage, but should never exceed entity number
	{ PSF102(saberCanThrow), 1 },
	{ PSF102(forceHandExtend), 8 },
	{ PSF102(forceDodgeAnim), 16 },
	{ PSF102(fd.saberAnimLevel), 2 },
	{ PSF102(fd.saberDrawAnimLevel), 2 },
	{ PSF102(saberAttackChainCount), 4 },
	{ PSF102(saberHolstered), 1 },
	{ PSF102(usingATST), 1 },
	{ PSF102(atstAltFire), 1 },

	{ PSF102(duelIndex), GENTITYNUM_BITS },
	{ PSF102(duelTime), 32 },
	{ PSF102(duelInProgress), 1 },

	{ PSF102(saberLockTime), 32 },
	{ PSF102(saberLockEnemy), GENTITYNUM_BITS },
	{ PSF102(saberLockFrame), 16 },
	{ PSF102(saberLockAdvance), 1 },

	{ PSF102(inAirAnim), 1 }, //just transmit it for the sake of knowing right when on the client to play a land anim, it's only 1 bit
	{ PSF102(dualBlade), 1 },

	{ PSF102(lastHitLoc[2]), 0 },
	{ PSF102(lastHitLoc[0]), 0 },
	{ PSF102(lastHitLoc[1]), 0 } //currently only used so client knows to orient disruptor disintegration.. seems a bit much for just that though.
};

netField_t	playerStateFields104[] = 
{
{ PSF(commandTime), 32 },				
{ PSF(origin[0]), 0 },
{ PSF(origin[1]), 0 },
...
void MSG_WriteDeltaPlayerstate( msg_t *msg, struct playerState_s *from, struct playerState_s *to ) {
	...
	if (JK2MF_GetGameVersion() == JK2_102) {
		numFields = sizeof(playerStateFields102) / sizeof(playerStateFields102[0]);
	}
	else {
		numFields = sizeof(playerStateFields104) / sizeof(playerStateFields104[0]);
	}

	lc = 0;
	if (JK2MF_GetGameVersion() == JK2_102)
		field = playerStateFields102;
	else
		field = playerStateFields104;
...
void MSG_ReadDeltaPlayerstate (msg_t *msg, playerState_t *from, playerState_t *to ) {
	...
	*to = *from;
	...
	if (JK2MF_GetGameVersion() == JK2_102)
		field = &playerStateFields102[lc];
	else
		field = &playerStateFields104[lc];
