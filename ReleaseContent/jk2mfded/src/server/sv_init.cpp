...
void SV_Init (void) {
	...
	if ( JK2MF_GetGameVersion() == JK2_102 )
		Cvar_Get ("protocol", va("%i", PROTOCOL_VERSION102), CVAR_SERVERINFO | CVAR_ROM);
	else
		Cvar_Get("protocol", va("%i", PROTOCOL_VERSION104), CVAR_SERVERINFO | CVAR_ROM);
	...
	sv_master[0] = Cvar_Get("sv_master1", MASTER_SERVER_NAME, CVAR_ROM);
	sv_master[1] = Cvar_Get("sv_master2", "master.ouned.de", CVAR_ROM);
