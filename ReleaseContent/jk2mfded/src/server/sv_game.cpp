...
void SV_GameSendServerCommand(int clientNum, const char *text) {
	if (JK2MF_InUse())
		JK2MF_SV_GameSendServerCommand(clientNum, text);
	else
		SV_GameSendServerCommand_Ori(clientNum, text);
}

void SV_GameSendServerCommand_Ori( int clientNum, const char *text ) {
	if ( clientNum == -1 ) {
...
void SV_LocateGameData(sharedEntity_t *gEnts, int numGEntities, int sizeofGEntity_t,
	...
	if (JK2MF_InUse())
		JK2MF_SV_LocateGameData(gEnts, numGEntities, sizeofGEntity_t, clients, sizeofGameClient);
	else
		SV_LocateGameData_Ori(gEnts, numGEntities, sizeofGEntity_t, clients, sizeofGameClient);
}

void SV_LocateGameData_Ori( sharedEntity_t *gEnts, int numGEntities, int sizeofGEntity_t,
					   playerState_t *clients, int sizeofGameClient ) {
	sv.gentities = gEnts;
...
void SV_GetUsercmd(int clientNum, usercmd_t *cmd) {
	if (JK2MF_InUse())
		JK2MF_SV_GetUsercmd(clientNum, cmd);
	else
		SV_GetUsercmd_Ori(clientNum, cmd);
}

void SV_GetUsercmd_Ori( int clientNum, usercmd_t *cmd ) {
	if ( clientNum < 0 || clientNum >= sv_maxclients->integer ) {
...
int SV_GameSystemCalls(int *args) {
	if (JK2MF_InUse())
		return JK2MF_SV_GameSystemCalls(args);
	else
		return SV_GameSystemCalls_Ori(args);
}

int SV_GameSystemCalls_Ori( int *args ) {
	// translate the changed ones back
	if (JK2MF_GetGameVersion() == JK2_102) {
		if (args[0] > G_G2_GETBOLT_NOREC) {
			args[0]++;
		}
	}
