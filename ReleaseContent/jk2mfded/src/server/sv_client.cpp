...
void SV_DirectConnect( netadr_t from ) {
	...
	if (!((JK2MF_GetGameVersion() == JK2_102 && version == PROTOCOL_VERSION102) || (JK2MF_GetGameVersion() == JK2_104 && version == PROTOCOL_VERSION104))) {
		NET_OutOfBandPrint(NS_SERVER, from, "print\nServer uses protocol version %i.\n", (JK2MF_GetGameVersion() == JK2_102) ? PROTOCOL_VERSION102 : PROTOCOL_VERSION104);
...
void SV_BeginDownload_f(client_t *cl) {
	if (JK2MF_InUse())
		JK2MF_SV_BeginDownload(cl);
	else
		SV_BeginDownload_f_Ori(cl);
}

void SV_BeginDownload_f_Ori( client_t *cl ) {
	...
	// ouned: q3dirtrav fix
	unsigned int len = strlen(cl->downloadName);
	if (len <= 4 || strstr(cl->downloadName, "..") || cl->downloadName[len - 4] != '.' || cl->downloadName[len - 3] != 'p' || cl->downloadName[len - 2] != 'k' || cl->downloadName[len - 1] != '3') {
		SV_CloseDownload(cl);
	}
}
...
void SV_UpdateUserinfo_f(client_t *cl) {
	if (JK2MF_InUse())
		JK2MF_SV_UpdateUserinfo(cl);
	else
		SV_UpdateUserinfo_f_Ori(cl);
}

void SV_UpdateUserinfo_f_Ori( client_t *cl ) {
...
void SV_ExecuteClientCommand(client_t *cl, const char *s, qboolean clientOK) {
	if (JK2MF_InUse())
		JK2MF_SV_ExecuteClientCommand(cl, s, clientOK);
	else
		SV_ExecuteClientCommand_Ori(cl, s, clientOK);
}

void SV_ExecuteClientCommand_Ori( client_t *cl, const char *s, qboolean clientOK ) {
	ucmd_t	*u;
